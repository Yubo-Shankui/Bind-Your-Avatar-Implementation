...
def check_mask_left(mask_np):
    h, w = mask_np.shape[:2]
    mid = w // 2
    left_sum = np.sum(mask_np[:, :mid])
    right_sum = np.sum(mask_np[:, mid:])
    if left_sum > right_sum:
        return 'left'
    elif right_sum > left_sum:
        return 'right'
    else:
        return 'equal_or_none'
...
def get_valid_segments(valid_frame, tolerance=5):
    # valid_positions = sorted(set(valid_frame['face']).union(set(valid_frame['head']))) # Do we need to add 'person'?
    try:
        valid_positions = sorted(set(valid_frame['face']).union(set(valid_frame['head'])))
    except KeyError:
        try:
            valid_positions = sorted(set(valid_frame['face']))
        except KeyError:
            valid_positions = sorted(set(valid_frame['head']))
...
def generate_frame_indices_for_face(n_frames, sample_stride, valid_frame, tolerance=7, skip_frames_start_percent=0.0, skip_frames_end_percent=1.0, skip_frames_start=0, skip_frames_end=0):
    ...
    if valid_length <= n_frames:
        ...
        raise ValueError("video_length:{} <= n_frames:{}".format(valid_length, n_frames))
    else:
...
def select_mask_frames_from_index(batch_frame, original_batch_frame, valid_id, corresponding_data, control_sam2_frame,
    ...
    """
    Directly takes all frames without distance or random constraints.
    If any frame does not exist in valid_frame[mask_type], it will raise an error.
    """
    # Use all frames from original_batch_frame directly and keep the original order
    selected_frames = original_batch_frame

    # Check if mask and bbox exist for each frame, raise an error if any are missing
    selected_masks_dict = {}
    selected_bboxs_dict = {}
    dense_masks_dict = {}
    ...
    if dense_masks:
        for frame in original_batch_frame:
            # track_id = int(valid_id) * 3 - 0
            # print(f"now track_id: {track_id}") # 3 6
            try:
                track_id = corresponding_data[valid_id]['face']
            except KeyError:
                try:
                    track_id = corresponding_data[valid_id]['head']
                except KeyError:
                    track_id = corresponding_data[valid_id]['person']

            # print(f"updated track_id: {track_id}") # 1 4
            mask_data_path = f"{base_dir}/{track_id}/annotated_frame_{int(frame):05d}.png"
    ...
    for frame in selected_frames:
        if frame not in valid_frame[mask_type]:
            raise ValueError(f"Missing mask data: Frame {frame} does not exist in valid_frame[{mask_type}]")
        # track_id = int(valid_id) * 3 - 1
        # selected_frames is probably not used
        try:
            track_id = corresponding_data[valid_id]['face']
        except KeyError:
            try:
                ...
                track_id = corresponding_data[valid_id]['person']
        mask_data_path = f"{base_dir}/{track_id}/annotated_frame_{frame:05d}.png"
        mask_array = np.array(Image.open(mask_data_path))
        binary_mask = np.where(mask_array > 0, 1, 0).astype(np.uint8)
        ...
        try:
            ...
            print(f"temp_bbox cannot be accessed,frame: {frame}, valid_id: {valid_id}")
...
class ConsisID_Dataset(Dataset):
    def __init__(
            ...
            width=640,
            max_num_frames=49,
            sample_stride=1, # Continuous sampling
            skip_frames_start_percent=0.0,
            skip_frames_end_percent=1.0,
            skip_frames_start=2, # Adapt to audio windowing. 2 without inpainting, 1 with inpainting.
            skip_frames_end=2,
            text_drop_ratio=-1,
            is_train_face=False,
        ...
        self.instance_speaker = []
        for sub_root, anno, anno_base in tqdm(folder_anno):
            print(anno)
            with open(anno, 'r') as f:
                sub_list = json.load(f)
            for i in tqdm(sub_list):
                path = os.path.join(sub_root, os.path.basename(i['path']) + ".mp4")
                cap = i.get('cap', None)
                fps = i.get('fps', 0)
                duration = i.get('duration', 0)
                speaker = i.get('speaker', "left")
                ...
                self.instance_speaker.append(speaker)
    ...
    def _get_frame_indices_adjusted(self, video_length, n_frames): #repeat to complete the clip length
    ...
    def _generate_frame_indices(self, video_length, n_frames, sample_stride, skip_frames_start_percent=0.0, skip_frames_end_percent=1.0, skip_frames_start=0, skip_frames_end=0):
        ...
        if video_length <= n_frames:
            ...
            raise ValueError("video_length:{} <= n_frames:{}".format(video_length, n_frames))
            ...
            clip_length = min(adjusted_length, (n_frames - 1) * sample_stride + 1) #The case where less than n_frames can be taken
    ...
    def _short_resize_and_crop(self, frames, target_width, target_height):
        ...
        # Pad to target size (center pad with black borders)
        pad_left = (target_width - new_width) // 2
        pad_right = target_width - new_width - pad_left
        pad_top = (target_height - new_height) // 2
        pad_bottom = target_height - new_height - pad_top

        padded_frames = F.pad(resized_frames, (pad_left, pad_right, pad_top, pad_bottom), value=0)
    
        sample = padded_frames.permute(0, 2, 3, 1)
    ...
    def _get_af_matrix(self, idx,corresponding_data,valid_id,base_dir):
        speaker = self.instance_speaker[idx]
        assert speaker is not None, "speaker is None"
        assert speaker != "None", "speaker is 'None'"
        # Read mask to determine if f1 is on the left or right
        frame=0
        try:
            track_id = corresponding_data[valid_id]['face']
        except KeyError:
        ...
        # print(f"updated track_id: {track_id}") # 1 4
        mask_data_path = f"{base_dir}/{track_id}/annotated_frame_{int(frame):05d}.png"
        mask = Image.open(mask_data_path)
        mask_np = np.array(mask)
        is_mask_left = check_mask_left(mask_np)
        # If they are equal, it's an identity matrix, otherwise it's 1 - identity matrix, shape is (2,2)
        if is_mask_left == speaker:
    ...
    def get_batch(self, idx):
        ...
        with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
            ...
            if self.is_train_face:
                ...
                if self.is_train_audio:
                    audio_emb_base_name = os.path.basename(video_dir.replace(".mp4", ".pt"))
                    audio_emb_path = os.path.join(anno_base_path, "audio_emb", audio_emb_base_name)
                    audio_emb_left_path = os.path.join(anno_base_path, "audio_emb","left_audio", audio_emb_base_name)
                    audio_emb_right_path = os.path.join(anno_base_path, "audio_emb","right_audio", audio_emb_base_name)
                ...
                if self.is_single_face:
                    ...
                    assert len(corresponding_data) <= 2, f"Using one or double face, but number of faces is {len(corresponding_data)}"

                # Support dual IDs: get all valid IDs
                valid_ids = []
                for id_key, data in corresponding_data.items():
                    # print(data['face'])
                    if 'face' in data or 'head' in data:
                        valid_ids.append(id_key)

                # valid_ids = [k for k in corresponding_data.keys() if k in valid_frame]
                if not valid_ids:
                    raise ValueError("No valid IDs")

                total_index = list(range(video_num_frames))
                # Generate batch_index for each id separately
                batch_index_dict = {}
                
                global_batch_index, _ = generate_frame_indices_for_face(
                    self.max_num_frames, self.sample_stride, valid_frame[valid_ids[0]],
                ...
                # Each id's batch_index uses the global sequence
                batch_index_dict = {vid: global_batch_index for vid in valid_ids}
                
                # Select mask data, keeping the is_cross_face branch logic
                selected_frame_index_dict = {}
                selected_masks_dict_dict = {}
                selected_bboxs_dict_dict = {}
                dense_masks_dict_dict = {}
                reserve_face_imgs_dict = {}
                reserve_frame_index_dict = {}
                reserve_bboxs_dict_dict = {}
                for vid in valid_ids:
                    if self.is_cross_face:
                        remaining_batch_index = [i for i in total_index if i not in global_batch_index]
                        try:
                            _sel_frame_idx, _sel_masks, _sel_bboxs, _dense_masks = select_mask_frames_from_index(
                                remaining_batch_index,
                                global_batch_index,
                                vid,
                                corresponding_data,
                                control_sam2_frame,
                    ...
                    selected_frame_index_dict[vid] = _sel_frame_idx[vid]
                    selected_masks_dict_dict[vid] = _sel_masks[vid]
                    selected_bboxs_dict_dict[vid] = _sel_bboxs[vid]
                    dense_masks_dict_dict[vid] = _dense_masks[vid]
                    if self.is_reserve_face:
                        _res_frame_idx, _, _res_bboxs, _ = select_mask_frames_from_index(
                            global_batch_index,
                            global_batch_index,
                            vid,
                            corresponding_data,
                        ...
                        reserve_frame_index_dict[vid] = _res_frame_idx[vid]
                        reserve_bboxs_dict_dict[vid] = _res_bboxs[vid]
                
                # Get crop and face processing results for each id separately
                expand_face_imgs_dict = {}
                original_face_imgs_dict = {}
                for vid in valid_ids:
                    _expand_pil, _original_pil = crop_images(
                        selected_frame_index_dict[vid], selected_bboxs_dict_dict[vid],
                        video_reader, return_ori=True
                    )
                    _expand_imgs, _original_imgs = process_cropped_images(_expand_pil, _original_pil, target_size=(480,480))
                    expand_face_imgs_dict[vid] = _expand_imgs
                    original_face_imgs_dict[vid] = _original_imgs
                
                # Process dense_masks_tensor: concatenate and crop for each id separately
                dense_masks_tensor_dict = {}
                for vid in valid_ids:
                    temp_dense = torch.stack([torch.tensor(mask) for mask in dense_masks_dict_dict[vid]])
                    dt = self._short_resize_and_crop(temp_dense.unsqueeze(-1), self.width, self.height).squeeze(-1)
                    dense_masks_tensor_dict[vid] = dt
                
                # If reserve_face_imgs is needed, process each id similarly
                if self.is_reserve_face:
                    reserve_face_imgs_processed = {}
                    for vid in valid_ids:
                        _res_pil, _ = crop_images(
                            reserve_frame_index_dict[vid], reserve_bboxs_dict_dict[vid],
                            video_reader, return_ori=False
                        )
                        _res_imgs, _ = process_cropped_images(_res_pil, [], target_size=(480,480))
                        reserve_face_imgs_processed[vid] = _res_imgs
                    reserve_face_imgs_dict = reserve_face_imgs_processed
                
                for vid in valid_ids:
                    if len(expand_face_imgs_dict[vid]) == 0 or len(original_face_imgs_dict[vid]) == 0:
                        raise ValueError("No face detected in input image pool")

                # Pad the results for each id
                # This padding is actually to unify the shape. If max_frames is 1, it will only keep the first image for each id in each video.
                for vid in valid_ids:
                    expand_face_imgs_dict[vid] = pad_tensor(expand_face_imgs_dict[vid], self.max_frames, dim=0)
                    original_face_imgs_dict[vid] = pad_tensor(original_face_imgs_dict[vid], self.max_frames, dim=0)
                    tensor_idx = torch.tensor(selected_frame_index_dict[vid])
                    selected_frame_index_dict[vid] = pad_tensor(tensor_idx, self.max_frames, dim=0)

                activate_id = None
                if len(valid_ids) == 1:
                    try:
                        # Ensure the current id is "1", add "2"
                        assert valid_ids[0] == "1", f"Single person ID should be '1', but got {valid_ids[0]}"
                        valid_ids.append("2")

                        # Randomly decide which id uses a zeros mask
                        if random.random() < 0.5:
                            activate_id = "1"
                            # "1" remains as is, "2" uses zeros
                            dense_masks_tensor_dict["2"] = torch.zeros_like(dense_masks_tensor_dict["1"])
                        else:
                            activate_id = "2"
                            # "2" copies data from "1", "1" uses zeros
                            original_mask = dense_masks_tensor_dict["1"]
                            dense_masks_tensor_dict["2"] = original_mask.clone()
                            dense_masks_tensor_dict["1"] = torch.zeros_like(original_mask)

                        # Copy other data
                        expand_face_imgs_dict["2"] = expand_face_imgs_dict["1"].clone()
                        if "1" in reserve_face_imgs_dict:
                            reserve_face_imgs_dict["2"] = reserve_face_imgs_dict["1"].clone()
                        if "1" in original_face_imgs_dict:
                            original_face_imgs_dict["2"] = original_face_imgs_dict["1"].clone()
                        selected_frame_index_dict["2"] = selected_frame_index_dict["1"].clone()
                    except Exception as e:
                        print(f"Error in single person data processing for {idx}")
            ...
            try:
                ...
                audio_embs = None
                if self.is_train_audio:
                    start_index=batch_index[0]
                    end_index=batch_index[-1]
                    try:#single-channel audio
                        audio_embs=torch.load(audio_emb_path)[[start_index-2,start_index-1]+batch_index+[end_index+1,end_index+2]]
                    except:
                        try:#dual-channel audio
                            audio_embs_left=torch.load(audio_emb_left_path)[[start_index-2,start_index-1]+batch_index+[end_index+1,end_index+2]]
                            audio_embs_right=torch.load(audio_emb_right_path)[[start_index-2,start_index-1]+batch_index+[end_index+1,end_index+2]]
                        except:
                            audio_embs_left=torch.load(audio_emb_left_path)[start_index-2:]
                            audio_embs_right=torch.load(audio_emb_right_path)[start_index-2:]
                            assert audio_embs_left.shape[0] <= self.max_num_frames+4 and audio_embs_right.shape[0] <= self.max_num_frames+4
                            num_frames_to_pad = self.max_num_frames+4-audio_embs_left.shape[0]
                            num_frames_to_pad_right = self.max_num_frames+4-audio_embs_right.shape[0]
                            audio_embs_left = torch.cat([audio_embs_left,torch.zeros(num_frames_to_pad,12,768)])
                            audio_embs_right = torch.cat([audio_embs_right,torch.zeros(num_frames_to_pad_right,12,768)])
                        audio_embs=torch.cat([audio_embs_left.unsqueeze(0),audio_embs_right.unsqueeze(0)],dim=0) # torch.shape(2,frames,12,768)
                    # torch.shape(frames,12,768)
                    # unsqueeze and repeat audio_embs twice to get shape (2, frames, 12, 768)
                    # audio_embs = audio_embs.unsqueeze(0).repeat(2,1,1,1)
                    # audio_embs = audio_embs.unsqueeze(0)
                    if activate_id is not None:
                        af_matrix = torch.eye(2) if activate_id == "1" else 1 - torch.eye(2)
                    else:
                        af_matrix = self._get_af_matrix(idx,corresponding_data,valid_ids[0],masks_data_path)
                else:
                    af_matrix = None
                    audio_embs = None
        ...
        if self.is_train_face:
            return (pixel_values, text, 'video', video_dir,
                    expand_face_imgs_dict, dense_masks_tensor_dict,
                    selected_frame_index_dict, reserve_face_imgs_dict, original_face_imgs_dict,audio_embs,af_matrix)
        else:
            return pixel_values, text, 'video', video_dir
    ...
    def __getitem__(self, idx):
        while True:
            ...
            try:
                if self.is_train_face:
                    pixel_values, cap, data_type, video_dir, expand_face_imgs_dict, dense_masks_tensor_dict, selected_frame_index_dict, reserve_face_imgs_dict, original_face_imgs_dict,audio_embs,af_matrix = self.get_batch(idx)
                    sample["instance_prompt"] = self.id_token + cap
                    sample["instance_video"] = pixel_values
                    sample["video_path"] = video_dir
                    if self.is_train_face:
                        sample["expand_face_imgs"] = expand_face_imgs_dict
                        sample["dense_masks_tensor"] = dense_masks_tensor_dict
                        sample["selected_frame_index"] = selected_frame_index_dict
                        if reserve_face_imgs_dict is not None:
                            sample["reserve_face_imgs"] = reserve_face_imgs_dict
                        if original_face_imgs_dict is not None:
                            sample["original_face_imgs"] = original_face_imgs_dict
                    if self.is_train_audio:
                        sample["audio_embs"] = audio_embs
                        sample["af_matrix"] = af_matrix
