...
from models.pipeline_bindyouravatar import BindyouravatarPipeline, draw_kps
...
def main(args):
    ...
    load_model_log_path = os.path.join(args.output_dir, "train_load_model.log")
    ...
    if accelerator.is_main_process:
        if args.output_dir is not None:
            ...
            with open(load_model_log_path, 'w') as f:
                f.write("")  
    ...
    transformer_additional_kwargs={
        'is_train_audio': args.is_train_audio,
        'is_teacher_forcing': args.is_teacher_forcing,
        'in_channels': args.in_channels,
    }
    with open(load_model_log_path, 'a') as f:
        with redirect_stdout(f):
            transformer = BindyouravatarTransformer3DModel.from_pretrained_cus(
    ...
    if args.use_ema:
        ema_transformer3d = BindyouravatarTransformer3DModel.from_pretrained_cus(
            args.pretrained_model_name_or_path if args.pretrained_weight is None else args.pretrained_weight,
            subfolder="transformer",
            config_path=args.config_path,
            transformer_additional_kwargs=transformer_additional_kwargs,
        ...
        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=BindyouravatarTransformer3DModel, model_config=ema_transformer3d.config)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for i, model in enumerate(models):
                ...
                model.save_router_modules(os.path.join(output_dir, "router_modules.pt"))
                if args.is_train_audio:
                    model.save_audio_modules(os.path.join(output_dir, "audio_modules.pt"))
                if weights:
                    weights.pop()

            if args.is_train_lora:
                BindyouravatarPipeline.save_lora_weights(
                    output_dir,
    ...
    def load_model_hook(models, input_dir):
        # resume的时候还要改后面的代码，857 行
        with open(load_model_log_path, 'a') as f:
            with redirect_stdout(f):
                if args.use_ema:
                    ema_path = os.path.join(input_dir, "transformer_ema")
                    _, ema_kwargs = BindyouravatarTransformer3DModel.load_config(ema_path, return_unused_kwargs=True)
                    load_model = BindyouravatarTransformer3DModel.from_pretrained_cus(
                        input_dir,
                        subfolder="transformer_ema",
                        transformer_additional_kwargs=transformer_additional_kwargs,
                    )

                    load_model = EMAModel(load_model.parameters(), model_cls=BindyouravatarTransformer3DModel, model_config=load_model.config)
                ...
                for i in range(len(models)):
                    ...
                    if args.is_train_lora:
                        lora_state_dict = BindyouravatarPipeline.lora_state_dict(input_dir)
    ...
    if args.is_train_lora:
        transformer_lora_config = LoraConfig(
            ...
            target_modules=["attn1.to_k", "attn1.to_q"],
            # modules_to_not_convert=["router", "local_facial_extractor", "perceiver_cross_attention"] # 添加这一行
        )
        transformer.add_adapter(transformer_lora_config)
        
    # load pretrained module
    if args.load_pretrained_module and accelerator.is_main_process:
        pretrained_base_path = args.pretrained_model_name_or_path if args.pretrained_weight is None else args.pretrained_weight,
        pretrained_base_path = pretrained_base_path[0]
        
        audio_model_basename = "audio_modules.pt"
        face_model_basename = "face_modules.pt"
        lora_basename = "pytorch_lora_weights.safetensors"
        router_basename = "router_modules.pt"
        for basename in args.load_pretrained_modules_list_path:
            if "audio_module" in basename:
                audio_model_basename = basename
            if "face_module" in basename:
                face_model_basename = basename
            if "lora_weight" in basename:
                lora_basename = basename
            if "router" in basename:
                router_basename = basename
        audio_model_path = os.path.join(pretrained_base_path, "audio_module", audio_model_basename)
        face_model_path = os.path.join(pretrained_base_path, "face_module", face_model_basename)
        lora_path = os.path.join(pretrained_base_path, "lora_weight", lora_basename)
        router_path = os.path.join(pretrained_base_path, "router", router_basename)
        
        with open(load_model_log_path, 'a') as f:
            with redirect_stdout(f):
                print(f"---------------------args.load_pretrained_modules_list_path: {args.load_pretrained_modules_list_path}---------------------")
                print(f"-------------------------------load_pretrained_modules: {args.load_pretrained_modules_list}--------------------------------")
                if "audio_module" in args.load_pretrained_modules_list:
                ...
                if "face_module" in args.load_pretrained_modules_list:
                ...
                if "lora_weight" in args.load_pretrained_modules_list:
                    print(f"--------------------------------start loading lora-----------------------------------------------")
                    print(f"--------------------------------lora_path: {lora_path}--------------------------------------")
                    lora_weights = load_file(lora_path)
                    lora_weights = {k.replace("transformer.module.", ""): v  for k, v in lora_weights.items()}
                    ...
                    print(f"--------------------------------complete loading lora--------------------------------")    
                if "router" in args.load_pretrained_modules_list:
                    print(f"--------------------------------start loading router-----------------------------------------------")
                    print(f"--------------------------------router_path: {router_path}--------------------------------------")
                    transformer.load_router_modules(router_path, strict=False)
                    print(f"--------------------------------complete loading router-------------------------------------------")


    unfreeze_modules = args.unfreeze_modules
    ...
    for module_name in unfreeze_modules:
        def getattr_recursive(obj, attr_path):
            ...
            return obj
        try:
            module_to_train = getattr_recursive(transformer, module_name)
            if isinstance(module_to_train, torch.nn.Parameter):
                module_to_train.requires_grad = True
            else:
            ...
            with open(load_model_log_path, 'a') as f:
                with redirect_stdout(f):
                    print(f"Module {module_name} not found in transformer,error is {e}")
            # raise AttributeError(f"Module {module_name} not found in transformer")
            continue

    for name, param in transformer.named_parameters():
        for unfreeze_module_name in unfreeze_modules:
            if unfreeze_module_name in name:
                param.requires_grad = True
                break

    for name, param in transformer.named_parameters():
        for freeze_module_name in freeze_modules:
            if freeze_module_name in name:
    ...
    if accelerator.is_main_process:
        with open(load_model_log_path, 'a') as f:
            with redirect_stdout(f):
                ...
                for name, param in transformer.named_parameters(): print(f"{name}: {param.requires_grad}") if param.requires_grad else None
        ...
        if path is None:
            ...
            if args.is_accelerator_state_dict:
                ...
                try:
                    ...
                    print(f"--------------------------------Applying new learning rate in accelerator state dict: {args.learning_rate} --------------------------------")
                    unwrapped_optimizer = accelerator.unwrap_model(optimizer)
                    for param_group in unwrapped_optimizer.param_groups:
                        old_lr = param_group['lr']
                        param_group['lr'] = args.learning_rate
                        print(f"Changed learning rate from {old_lr} to {param_group['lr']}")

                    unwrapped_scheduler = accelerator.unwrap_model(lr_scheduler)
                    if hasattr(unwrapped_scheduler, 'base_lrs'):
                        old_base_lrs = unwrapped_scheduler.base_lrs.copy() if hasattr(unwrapped_scheduler.base_lrs, 'copy') else list(unwrapped_scheduler.base_lrs)
                        new_base_lrs = [args.learning_rate] * len(unwrapped_scheduler.base_lrs)
                        unwrapped_scheduler.base_lrs = new_base_lrs
                        print(f"Updated scheduler base learning rates from {old_base_lrs} to {new_base_lrs}")
                except Exception as e:
                    with open(load_model_log_path, 'a') as f:
                        with redirect_stdout(f):
                            print(f"--------------------------------state dict path: {os.path.join(args.output_dir, path)} --------------------------------")
                            print(f"--------------------------------Failed to load accelerator state: {str(e)} --------------------------------")
                ...
                with open(load_model_log_path, 'a') as f:
                    with redirect_stdout(f):
                        print(f"--------------------------------found is_accelerator_state_dict={args.is_accelerator_state_dict} --------------------------------")
                        print(f"--------------------------------load model from {pretrained_model_path} --------------------------------")
                        m, u = accelerator.unwrap_model(transformer).load_state_dict(load_model.state_dict(), strict=False)

                        print(f"missing keys: {len(m)}, unexpected keys: {len(u)}")
                        print(f"missing keys: {m}, unexpected keys: {u}")
                        print(f"--------------------------------load lora from {pretrained_model_path} --------------------------------")
                        def load_lora_from_transformer_safetensors(transformer, transformer_path, lora_rank=128,flag_lora_ready=False):
                            from safetensors.torch import load_file
                            import json
                        ...
                        loaded_transformer = load_lora_from_transformer_safetensors(unwrapped_transformer, pretrained_model_path, flag_lora_ready=True)
                        transformer.load_state_dict(loaded_transformer.state_dict(),strict=False)
                        print(f"------------------------------complete load lora from {pretrained_model_path} --------------------------------")

                        optimizer_path = os.path.join(pretrained_model_path, "optimizer.pt")
                        scheduler_path = os.path.join(pretrained_model_path, "scheduler.pt")
                        if os.path.exists(optimizer_path):
                            
                            try:
                                optimizer_state = torch.load(optimizer_path)
                                if not isinstance(optimizer_state, dict):
                                    raise ValueError("Invalid optimizer state file format")
                                print(f"-----------------------------loading optimizer_state: {optimizer_state} --------------------------------")

                                unwrapped_optimizer = accelerator.unwrap_model(optimizer)
                                unwrapped_optimizer.load_state_dict(optimizer_state)
                        ...
                        if os.path.exists(scheduler_path):
                            try:
                                ...
                                if not isinstance(scheduler_state, dict):
                                    raise ValueError("Invalid scheduler state file format")
                                unwrapped_scheduler = accelerator.unwrap_model(lr_scheduler)
                                unwrapped_scheduler.load_state_dict(scheduler_state)
                                print(f"--------------------------------Successfully loaded scheduler state from {scheduler_path} --------------------------------")
                            except Exception as e:
                                print(f"--------------------------------Failed to load scheduler state: {str(e)} --------------------------------")
                                print("--------------------------------Continuing without scheduler state --------------------------------")
    ...
    def process_image(image, vae, dense_masks=None):
        std = 0.5 if dense_masks is None else 0.5
        mean = -3.0 if dense_masks is None else -1
        image_noise_sigma = torch.normal(mean=mean, std=std, size=(1,), device=image.device)
        image_noise_sigma = torch.exp(image_noise_sigma).to(dtype=image.dtype)
        noisy_image = torch.randn_like(image) * image_noise_sigma[:, None, None, None, None]
        if dense_masks is not None: # tensor[480, 720]
            batch_size, num_channels, num_frames, height, width = image.shape
            dense_masks = dense_masks.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # [1,1,1,480,720]
            temp_image = torch.zeros((batch_size, num_channels, num_frames, height, width), device=image.device)
            dense_masks = resize_mask(dense_masks, temp_image, process_first_frame_only=True)  
            dense_masks = dense_masks.to(image.device)  # [1,1,1,480,720]
            noisy_image = noisy_image * dense_masks

        input_image = image + noisy_image
        image_latent_dist = vae.encode(input_image.to(dtype=vae.dtype)).latent_dist

        if dense_masks is not None:
            img=input_image.squeeze(0).squeeze(1)
            img = (img + 1) / 2
            img=img.to(torch.float32)
            Image.fromarray((img.clamp(0, 1).cpu().numpy() * 255).astype(np.uint8).transpose(1,2,0)).save('tests/paste_img/noisy_image.png')
        return image_latent_dist

    def encode_video(video, get_image_latent=True,dense_masks=None):
        ...
        if get_image_latent:
            image_latent_dist = process_image(image, vae, dense_masks=dense_masks[0]) #dense_masks: tensor[49, 480, 720]
        else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            try:
                with timeout(args.step_timeout):
                    with accelerator.accumulate(models_to_accumulate):
                        ...
                        with torch.no_grad():
                            id_cond = None
                            id_vit_hidden = None
                            dense_masks = None #no need
                            face_kps_latents = None #no need

                            batch_instance_video = batch["instance_video"]
                            batch_instance_prompt = batch["instance_prompt"]

                            if args.is_train_audio:
                                batch_instance_audio = batch["audio_embs"]  
                                batch_instance_af_matrix = batch["af_matrix"]
                            else:
                                batch_instance_audio = None
                                batch_instance_af_matrix = None

                            if args.is_train_face and args.enable_mask_loss and random.random() < args.mask_prob:
                            ...
                            if args.is_train_face:
                                face_ids = list(batch["expand_face_imgs"].keys())
                                expand_face_imgs_dict = batch['expand_face_imgs'] if batch['expand_face_imgs'] is not None else None                  # dict :[B, self.max_frames, C=3, H=112, W=112]
                                if args.enable_mask_loss:
                                    #  -----------------------------combine dense_masks to single mask for mask loss -----------------------------
                                    # get dense masks for all face ids
                                    dense_masks_combined = None 
                                    for face_id in face_ids:
                                        current_mask = batch['dense_masks_tensor'][face_id].to(memory_format=torch.contiguous_format).to(dtype=weight_dtype)#torch.Size([1, 49, 480, 720])
                                        if dense_masks_combined is None:
                                            dense_masks_combined = current_mask
                                        else:
                                            # use logical OR to merge masks
                                            dense_masks_combined = torch.logical_or(dense_masks_combined, current_mask).float()
                                    
                                    dense_masks = dense_masks_combined  # [B, T, H, W]#torch.Size([1, 49, 480, 720])
                                    # dense_masks = dense_masks.unsqueeze(1)  # [B, 1, T, H, W] torch.Size([1, 1, 49, 480, 720])

                                    # -----------------------------create index_mask for router loss -----------------------------
                                    # initialize index_mask, background is -1
                                    # dense_masks:[B, T, H, W]
                                    B = batch['dense_masks_tensor'][face_ids[0]].shape[0]
                                    T = (args.max_num_frames - 1 ) // (VAE_SCALE_FACTOR_SPATIAL // 2) +1 #13
                                    H = args.height // VAE_SCALE_FACTOR_SPATIAL #60
                                    W = args.width // VAE_SCALE_FACTOR_SPATIAL #90
                                    p = model_config.patch_size  
                                    
                                    masks_by_id = []
                                    for i, face_id in enumerate(face_ids):
                                        current_mask = batch['dense_masks_tensor'][face_id].to(memory_format=torch.contiguous_format).to(dtype=weight_dtype)
                                        current_mask = current_mask.unsqueeze(1)  # [B, 1, T, H, W]
                                        masks_by_id.append((current_mask, i))  

                                    fake_latent = torch.zeros((B, 1, T, H//p, W//p), device=current_mask.device)
                                    
                                    index_mask = torch.full((B, 1, T, H//p, W//p), -1, dtype=torch.long, device=current_mask.device)
                                    
                                    for mask, label in masks_by_id:
                                        resized_mask = resize_mask(mask, fake_latent, process_first_frame_only=False)  # [B, 1, T, H//p, W//p]
                                        binary_mask = (resized_mask > 0.5).long()
                                        index_mask = torch.where(binary_mask == 1, torch.tensor(label, device=binary_mask.device, dtype=torch.long), index_mask)
                                ...
                                if face_clip_model is not None and face_main_model is not None:
                                    B, T = expand_face_imgs_dict[face_ids[0]].shape[:2]
                                    # B_valid_num = torch.zeros(B, dtype=torch.int32)
                                    for face_id in face_ids:
                                        expand_valid_face_imgs = []
                                        original_valid_face_imgs = []
                                        # print(f'expand_face_imgs_dict[face_id].shape={expand_face_imgs_dict[face_id].shape}')
                                        for i in range(B):
                                            valid_mask = torch.any(torch.any(torch.any(expand_face_imgs_dict[face_id][i] != 0, dim=1), dim=1), dim=1) #torch.Size([2])当num_faces=2时，valid_mask=[True, True]
                                            # B_valid_num[i] = valid_mask.sum()
                                            expand_valid_face_imgs.extend(expand_face_imgs_dict[face_id][i][valid_mask]) # expand_face_imgs[i][valid_mask].shape=torch.Size([2, 3, 512, 512]) extend之后变成list,len=2(bs1*faces2)，每个元素shape=torch.Size([3, 512, 512])
                                            # original_valid_face_imgs.extend(original_face_imgs[i][valid_mask])
                                        expand_face_imgs_dict[face_id] = torch.stack(expand_valid_face_imgs)      # torch.Size([2, 3, 480, 480])
                                    ...
                                    valid_id_conds_1 = []
                                    valid_id_conds_2 = []
                                    valid_id_vit_hiddens_1 = []
                                    valid_id_vit_hiddens_2 = []
                                    valid_indices = []
                                    face_kps_list = []
                                    for idx, id_image in enumerate(expand_face_imgs_dict[face_ids[0]]):           # id_image: torch.Size([3, 480, 480]) 这里是对 bs 迭代，faces 的话按 dict 迭代
                                        ...
                                        try:
                                            id_images = []
                                            for face_id in face_ids:
                                                id_image = expand_face_imgs_dict[face_id][idx] #id_image for face_id, face_id=face_ids[0]时，等于enumerate 的id_image
                                            ...
                                            id_cond_list, id_vit_hidden_list, align_crop_face_image, face_kps,_ = process_face_embeddings_split(face_helper_1, face_clip_model, face_helper_2, eva_transform_mean, eva_transform_std, face_main_model, accelerator.device, weight_dtype, id_images, original_id_images=original_id_image, is_align_face=args.is_align_face)
                                        except Exception as e:
                                            processed = False
                                            ...
                                            if not processed:
                                                ...
                                                for frame_idx in range(0, Len_frame, 5):
                                                    try:
                                                        ...
                                                        id_image = resize_numpy_image_long(id_image, 1024)
                                                        id_cond, id_vit_hidden, align_crop_face_image, face_kps = process_face_embeddings( #从原视频裁，可以保证仍是 3 比 2
                                                        ...
                                                        id_cond_list = [id_cond for _ in range(len(face_ids))]
                                                        id_vit_hidden_list = [id_vit_hidden for _ in range(len(face_ids))]
                                                        processed = True
                                                        break
                                        ...
                                        if id_cond_list is not None:
                                            # for i in range(len(face_ids)):
                                            valid_id_conds_1.append(id_cond_list[0])
                                            valid_id_vit_hiddens_1.append(id_vit_hidden_list[0])
                                            valid_id_conds_2.append(id_cond_list[1])
                                            valid_id_vit_hiddens_2.append(id_vit_hidden_list[1])
                                            align_crop_face_imgs.append(align_crop_face_image)
                                            valid_indices.append(idx)
                                            if args.is_kps:
                                                face_kps_list.append(face_kps)

                                    if len(valid_id_conds_1) != 0:
                                        valid_id_conds_1 = torch.stack(valid_id_conds_1)
                                        valid_id_conds_2 = torch.stack(valid_id_conds_2)
                                        valid_id_conds_list = [valid_id_conds_1, valid_id_conds_2]
                                        valid_id_vit_hiddens_1 = [torch.cat(tensor_group, dim=0) for tensor_group in zip(*valid_id_vit_hiddens_1)]
                                        valid_id_vit_hiddens_2 = [torch.cat(tensor_group, dim=0) for tensor_group in zip(*valid_id_vit_hiddens_2)]
                                        valid_id_vit_hiddens_list = [valid_id_vit_hiddens_1, valid_id_vit_hiddens_2]
                                        align_crop_face_imgs = torch.stack(align_crop_face_imgs)

                                        if len(valid_id_conds_1) != B:
                                            ...
                                            if args.is_train_audio:
                                                batch_instance_audio = torch.stack([batch_instance_audio[i] for i in valid_indices])
                                                batch_instance_af_matrix = torch.stack([batch_instance_af_matrix[i] for i in valid_indices])
                                            print(f"Adjusted batch size from {B} to {len(valid_id_conds_1)} due to mismatched valid_id_conds length.")
                            ...
                            if not args.is_train_face or len(valid_indices) != 0:
                                ...
                                for idx, (video, face_img) in enumerate(zip(batch_instance_video, align_crop_face_imgs)):
                                    if args.is_train_face:
                                        latent_dist, image_latent_dist_full = encode_video(video, get_image_latent=True , dense_masks=dense_masks[idx])  # input [F, C, H, W] -> [B, C, F, H, W]
                                    ...
                                    if args.train_type == 'i2v':
                                        ...
                                        if image_latent_dist_full is not None:
                                            image_latents = torch.cat([image_latents, image_latent_dist_full.sample() * VAE_SCALING_FACTOR], dim=1)
                                        ...
                                        if args.is_kps:
                                            ...
                                            padding_shape = (video_latents.shape[0], video_latents.shape[1] - 2, image_latents.shape[2] , *video_latents.shape[3:])
                                        else:
                                            padding_shape = (video_latents.shape[0], video_latents.shape[1] - 1, image_latents.shape[2] , *video_latents.shape[3:])
                        ...
                        if not args.is_train_face or len(valid_indices) != 0:
                            ...
                            if patch_size_t is not None:
                                ...
                                padding_shape = (video_latents.shape[0], video_latents.shape[1] - image_latents.shape[1], image_latents.shape[2], * video_latents.shape[3:])
                                latent_padding = image_latents.new_zeros(padding_shape)
                                image_latents = torch.cat([image_latents, latent_padding], dim=1)
                            ...
                            if args.is_train_face and args.enable_mask_loss and enable_mask_loss_flag:
                                # way 1
                                dense_masks = dense_masks.unsqueeze(1)  # B F H W -> B 1 F H W    ->torch.Size([B=1, 1, 49, 480, 720])
                                temp_video_latents = torch.zeros((batch_size, num_channels, num_frames, height, width)) #torch.Size([B=1, 16, 13, 60, 90])
                                dense_masks = resize_mask(dense_masks, temp_video_latents, process_first_frame_only=False)  # torch.Size([B, 1, 13, 60, 90]),  B C F H W
                                if patch_size_t is not None: #is None
                                ...
                                dense_masks = dense_masks.repeat(1, temp_video_latents.shape[1], 1, 1, 1).permute(0, 2, 1, 3, 4).float()  #torch.Size([1, 13, 16, 60, 90]) B C F H W -> B F C H W
                                dense_masks = dense_masks.reshape(video_latents.shape[0], -1) #torch.Size([1, 1123200])
                            ...
                            valid_id_conds       = [valid_id_conds]*2
                            valid_id_vit_hiddens = [torch.zeros([1, 577, 1024]).to(accelerator.device, dtype=weight_dtype)] * 5
                            valid_id_vit_hiddens = [valid_id_vit_hiddens]*2

                        # Predict the noise residual
                        # Add random condition dropout logic - apply per sample in batch
                        batch_size = noisy_model_input.shape[0]
                        if args.drop_inpaint_prob > 0:
                            for i in range(batch_size):
                                if random.random() < args.drop_inpaint_prob:
                                    if args.train_type == 'i2v':
                                        # Zero out the image condition part (last 16 channels) for this sample
                                        noisy_model_input[i, :, 32:, :, :] = 0.0
                        
                        def create_custom_forward(module):
                            def custom_forward(hidden_states, encoder_hidden_states, timestep, image_rotary_emb, return_dict, id_cond, id_vit_hidden, audio_embeds, af_matrix, index_mask):
                                return module(
                        ...
                        for path_str in batch["video_path"]:
                            print(f"batch_instance_video_path {path_str}")

                        model_output, router_loss , consistency_loss, temporal_diff_loss , spatial_diff_loss, spatial_dist_loss, id_dist_loss = transformer(
                                hidden_states=noisy_model_input,
                                encoder_hidden_states=prompt_embeds,
                                timestep=timesteps,
                                image_rotary_emb=image_rotary_emb,
                                return_dict=False,
                                id_cond=valid_id_conds_list if valid_id_conds_list is not None else None,
                                id_vit_hidden=valid_id_vit_hiddens_list if valid_id_vit_hiddens_list is not None else None,
                                audio_embeds=batch_instance_audio,
                                af_matrix=batch_instance_af_matrix,
                                index_mask=index_mask,
                                index_mask_drop_prob=getattr(args, "index_mask_drop_prob", 0.0),
                                routing_logits_zeros_flag=random.random() < getattr(args, "routing_logits_zeros_prob", 0)
                            )
                        ...
                        if not args.is_train_face or len(valid_indices) != 0:
                            ...
                            if router_loss is not None:
                                router_loss_weight = getattr(args, "router_loss_weight", 0.1)  
                                loss = loss + router_loss_weight * router_loss
                            if consistency_loss is not None:
                                consistency_loss_weight = getattr(args, "consistency_loss_weight", 0.1)  
                                loss = loss + consistency_loss_weight * consistency_loss
                            if temporal_diff_loss is not None:
                                temporal_diff_loss_weight = getattr(args, "temporal_diff_loss_weight", 0.1)  
                                loss = loss + temporal_diff_loss_weight * temporal_diff_loss
                            if spatial_diff_loss is not None:
                                spatial_diff_loss_weight = getattr(args, "spatial_diff_loss_weight", 0.01)  
                                loss = loss + spatial_diff_loss_weight * spatial_diff_loss
                            if spatial_dist_loss is not None:
                                spatial_dist_loss_weight = getattr(args, "spatial_dist_loss_weight", 1)  
                                loss = loss + spatial_dist_loss_weight * spatial_dist_loss
                            if id_dist_loss is not None:
                                id_dist_loss_weight = getattr(args, "id_dist_loss_weight", 1)  
                                loss = loss + id_dist_loss_weight * id_dist_loss
                        else:
                    ...
                    if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                        if global_step % args.checkpointing_steps == 0:
                            ...
                            if args.is_accelerator_state_dict:
                                ...
                                unwrap_model(transformer).save_pretrained(os.path.join(save_path, "transformer"))
                                unwrap_model(transformer).save_face_modules(os.path.join(save_path, "face_modules.pt"))
                                unwrap_model(transformer).save_router_modules(os.path.join(save_path, "router_modules.pt"))
                                if args.is_train_audio:
                                    unwrap_model(transformer).save_audio_modules(os.path.join(save_path, "audio_modules.pt"))
                                if args.use_ema:
                                ...
                                if accelerator.is_main_process:
                                    try:
                                        unwrapped_optimizer = accelerator.unwrap_model(optimizer)
                                        optimizer_state = unwrapped_optimizer.state_dict()
                                        torch.save(optimizer_state, os.path.join(save_path, "optimizer.pt"))
                                        print(f"--------------------------------Successfully saved optimizer state to {save_path}/optimizer.pt --------------------------------")
                                    except Exception as e:
                                        print(f"--------------------------------Failed to save optimizer state: {str(e)} --------------------------------")

                                    try:
                                        unwrapped_scheduler = accelerator.unwrap_model(lr_scheduler)
                                        scheduler_state = unwrapped_scheduler.state_dict()
                                        torch.save(scheduler_state, os.path.join(save_path, "scheduler.pt"))
                                        print(f"--------------------------------Successfully saved scheduler state to {save_path}/scheduler.pt --------------------------------")
                                    except Exception as e:
                                        print(f"--------------------------------Failed to save scheduler state: {str(e)} --------------------------------")
                                        print("--------------------------------Continuing without scheduler state --------------------------------")
                            ...
                            if accelerator.is_main_process:
                                if args.validation_prompt is not None and args.is_validation:
                                    ...
                                    with torch.no_grad():
                                        # Create pipeline
                                        if args.train_type == 'i2v':
                                            pipe = BindyouravatarPipeline.from_pretrained(
            ...
            try:
                if router_loss is not None and consistency_loss is not None and temporal_diff_loss is not None and spatial_diff_loss is not None and spatial_dist_loss is not None and id_dist_loss is not None:
                    logs = {
                        ...
                        "temporal_diff_loss": temporal_diff_loss.detach().item(),
                        "spatial_diff_loss": spatial_diff_loss.detach().item(),
                        "spatial_dist_loss": spatial_dist_loss.detach().item(),
                        "id_dist_loss": id_dist_loss.detach().item()
                    }
                else:
                ...
                if router_loss is not None and consistency_loss is not None and temporal_diff_loss is not None and spatial_diff_loss is not None and spatial_dist_loss is not None and id_dist_loss is not None:
                    logs = {
                        ...
                        "temporal_diff_loss": temporal_diff_loss.item(),
                        "spatial_diff_loss": spatial_diff_loss.item(),
                        "spatial_dist_loss": spatial_dist_loss.item(),
                        "id_dist_loss": id_dist_loss.item()
                    }
                else:
                    logs = {"loss": 0.0}
    ...
    if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
        ...
        if args.is_accelerator_state_dict:
            ...
            transformer.save_face_modules(os.path.join(save_path, "face_modules.pt"))
            transformer.save_router_modules(os.path.join(save_path, "router_modules.pt"))
            if args.is_train_audio:
                transformer.save_audio_modules(os.path.join(save_path, "audio_modules.pt"))
        else:
            transformer.save_pretrained(os.path.join(save_path, "transformer"))
            transformer.save_face_modules(os.path.join(save_path, "face_modules.pt"))
            transformer.save_router_modules(os.path.join(save_path, "router_modules.pt"))
